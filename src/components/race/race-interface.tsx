...
const SUBTITLES_BY_STATE: Record<RaceState, string> = {
  idle: "// ENTER NICKNAME",
  lobby: "// AWAITING RIDERS",
  countdown: "// SIGNAL INCOMING",
  racing: "// BURN THE PROMPT",
    ...
    <div className="race-stage">
      <div className={`race-frame${isRacing ? " wide" : ""}`}>
        ...
        <div className="subtitle">
          {SUBTITLES_BY_STATE[state]}
        </div>

        {state === "idle" && <JoinDialog onJoinAction={handleJoin} />}

        {state === "lobby" && (
          <RaceLobby participants={lobbyParticipants}
            slots={3}
            currentParticipantId={participantId} />
        )}

        {state === "countdown" && startAt && (
          <TrafficLight startAt={startAt}
            onGoAction={handleGo} />
        )}
        ...
        {state === "racing" && (
          <>
            ...
            <TypingInput
              ...
              onSubmitAction={handleWordSubmit}
