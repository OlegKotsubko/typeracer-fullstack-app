  ...
  return (
    <>
      {NAV.map((n) => {
        ...
        return (
          <Link key={n.href} href={n.href} className={active ? "active" : ""} onClick={onClick}>
            {n.label}
          </Link>
        );
      })}
  ...
  return (
    <div className="adm">
      <aside className="adm-side">
        <Link href="/" className="adm-logo logo">
          <span>NEON</span>
      ...
      <div className={`adm-drawer${drawerOpen ? " open" : ""}`} onClick={() => setDrawerOpen(false)}>
        <div className="adm-drawer-inner" onClick={(e) => e.stopPropagation()}>
          <Link href="/" className="adm-logo logo">
            <span>NEON</span>
            <span style={{ color: "var(--green)" }}>DRIFT</span>
          </Link>
